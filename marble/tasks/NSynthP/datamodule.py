# marble/tasks/NSynthP/datamodule.py
...
class _NSynthPAudioBase(Dataset):
    """
    Base dataset for NSynth pitch classification audio.
    Each NSynth sample is a 4-second note at 16kHz (64000 samples).
    Label is the raw MIDI pitch (0-127).
    """
    ...
    def __getitem__(self, idx: int):
        ...
        label = int(info["label"])  # raw MIDI pitch as class index
...
class NSynthPAudioTrain(_NSynthPAudioBase):
    pass


class NSynthPAudioVal(_NSynthPAudioBase):
    pass


class NSynthPAudioTest(NSynthPAudioVal):
    pass


class NSynthPDataModule(BaseDataModule):
    pass


class NSynthPEmbeddingDataModule(pl.LightningDataModule):
    """DataModule for probing on pre-extracted NSynth pitch embeddings."""
    ...
    def setup(self, stage: str | None = None):
        if stage in (None, "fit"):
            self.train_dataset = NSynthPEmbeddingDataset(
                self.embedding_root / "train",
                self.layer_idx,
                self.train_jsonl,
            )
            self.val_dataset = NSynthPEmbeddingDataset(
