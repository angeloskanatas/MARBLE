# marble/tasks/NSynthP/embedding_dataset.py
...
# NSynth MIDI pitches range from 21 to 108, but we use 128 classes
# (full MIDI range 0-127) for a clean mapping: label = pitch.
NUM_PITCH_CLASSES = 128


class NSynthPEmbeddingDataset(Dataset):
    """
    ...
    def __init__(
        ...
        # label field in jsonl is the raw MIDI pitch (int)
        self._path_to_label = {
            info["audio_path"]: int(info["label"])
...
class NSynthPMultiLayerEmbeddingDataset(Dataset):
    """
    ...
    def __getitem__(self, idx: int):
        ...
        label = self._labels[idx]
        path = self._sample_to_audio_path[idx]
        return torch.stack(embs, dim=0), label, path  # (L, H)
