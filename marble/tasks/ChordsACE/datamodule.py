...
from marble.tasks.ChordsACE.embedding_dataset import (
    ChordsACEEmbeddingDataset,
    ChordsACEMultiLayerEmbeddingDataset,
)
...
class ChordsACEMultiLayerEmbeddingDataModule(pl.LightningDataModule):
    """Multi-layer DataModule: yields (B, L, T, H) batches by stacking layers."""
    ...
    def setup(self, stage: str | None = None):
        ...
        if stage in (None, "fit"):
            self.train_dataset = ChordsACEMultiLayerEmbeddingDataset(
                self.embedding_root / "train", jsonl=self.train_jsonl, **common
            )
            self.val_dataset = ChordsACEMultiLayerEmbeddingDataset(
                self.embedding_root / "val", jsonl=self.val_jsonl, **common
            )
        if stage in (None, "test", "predict"):
            self.test_dataset = ChordsACEMultiLayerEmbeddingDataset(
                self.embedding_root / "test", jsonl=self.test_jsonl, **common
            )
