...
from marble.tasks.GTZANBeatTracking.embedding_dataset import (
    GTZANBeatTrackingEmbeddingDataset,
    GTZANBeatTrackingMultiLayerEmbeddingDataset,
)
...
class GTZANBeatTrackingMultiLayerEmbeddingDataModule(pl.LightningDataModule):
    """
    DataModule for probing on pre-extracted GTZAN beat-tracking embeddings
    with MULTIPLE layers. Returns batches of shape (B, L, T, H).
    """
    ...
    def setup(self, stage: str | None = None):
        if stage in (None, "fit"):
            self.train_dataset = GTZANBeatTrackingMultiLayerEmbeddingDataset(
                embedding_dir=self.embedding_root / "train",
            ...
            self.val_dataset = GTZANBeatTrackingMultiLayerEmbeddingDataset(
                embedding_dir=self.embedding_root / "val",
                layer_indices=self.layer_indices,
                jsonl=self.val_jsonl,
                clip_seconds=self.clip_seconds,
        ...
        if stage in (None, "test", "predict"):
            self.test_dataset = GTZANBeatTrackingMultiLayerEmbeddingDataset(
                embedding_dir=self.embedding_root / "test",
                layer_indices=self.layer_indices,
                jsonl=self.test_jsonl,
                clip_seconds=self.clip_seconds,
