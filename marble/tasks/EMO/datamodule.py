...
from marble.tasks.EMO.embedding_dataset import EMOEmbeddingDataset
...
class EMOMultiLayerEmbeddingDataModule(pl.LightningDataModule):
    """
    DataModule for probing on pre-extracted EMO embeddings with MULTIPLE layers.
    ...
    def __init__(
        ...
        super().__init__()
        from marble.tasks.EMO.embedding_dataset import EMOMultiLayerEmbeddingDataset
        self._dataset_cls = EMOMultiLayerEmbeddingDataset
...
class EMOEmbeddingDataModule(pl.LightningDataModule):
    """
    DataModule for probing on pre-extracted EMO embeddings.
    """
    ...
    def setup(self, stage: str | None = None):
        if stage in (None, "fit"):
            self.train_dataset = EMOEmbeddingDataset(
                self.embedding_root / "train",
                self.layer_idx,
                self.train_jsonl,
            )
            self.val_dataset = EMOEmbeddingDataset(
                self.embedding_root / "val",
                self.layer_idx,
                self.val_jsonl,
            )
        if stage in (None, "test", "predict"):
            self.test_dataset = EMOEmbeddingDataset(
                self.embedding_root / "test",
                self.layer_idx,
                self.test_jsonl,
            )
