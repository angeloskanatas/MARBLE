...
EMB_ROOT=./output/extracted_embeddings_probing/maest_gtzan_genre
OUT=./output/probe.GTZANGenre.maest.layerwise
...
root = Path("./output/extracted_embeddings_probing/maest_gtzan_genre/train")
...
if [ "${#LAYERS[@]}" -eq 0 ]; then
  echo "No extracted layers found under $EMB_ROOT/train. Run extraction first (scripts/extract.MAEST.GTZANGenre.sh)."
  exit 1
fi
