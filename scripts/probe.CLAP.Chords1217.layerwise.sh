set -e
CONFIG=configs/probe.CLAP.Chords1217.layerwise.yaml
EMB_ROOT=./output/extracted_embeddings_probing/clap_chords1217
OUT=./output/probe.Chords1217.clap.layerwise
...
root = Path("./output/extracted_embeddings_probing/clap_chords1217/train")
...
data:
  init_args:
    label_freq: $FPS
EOF
  echo "Layer $layer (in_dim=$IN_DIM, median_T=$MEDIAN_T, fps=$FPS, target_frames=$((FPS * CLIP_SECONDS))): fit"
